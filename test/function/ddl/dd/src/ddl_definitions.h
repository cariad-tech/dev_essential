/**
 * @file
 * Headerfile for the tester for the DataDefinition representation.
 *
...
// Test DataDefinition that differs in the type of tBitmapFormat::nBytesPerLine
...
// Test DataDefinition that differs by names
...
// Test DataDefinition that differs by version
...
// Test DataDefinition where tBitmapFormat is only a subset
...
// Test DataDefinition where tMediaTypInfo is one element short
...
// Test DataDefinition with dynamic content
...
// Test DataDefinition with dynamic content but with different naming
...
// Test DataDefinition with dynamic content that is not binary equal (tBitmapFormat::nPixelFormat2)
...
// Test DataDefinition with dynamic content that is not binary equal in dynamic part
// (tMediaTypeInfo::ui32Flags has different type)
...
// Test DataDefinition to where the orignial struct layout is not the same (tTest1 vs tTest2) but
// the binary positions match
...
// Test DataDefinition to where the array size element is different
...
// Test DataDefinition that has special characters in name
...
static const std::string DDL_TEST_STRING_UNIT_USED_BUT_NOT_PRESENT = R"ESCAPE(
<?xml version="1.0" encoding="iso-8859-1" standalone="no"?>
<ddl:ddl xmlns:ddl="ddl">
    <structs>
        <struct alignment="1" name="the_struct" version="1">
            <element alignment="1" arraysize="8" byteorder="LE" bytepos="0" name="elem1" type="uint8_t" unit="my_unit_which_does_not_exist"/>
        </struct>
    </structs>
    <streams/>
</ddl:ddl>
