...
protected:
     ...
     * Determines how many bytes need to be copied to receive a copy of all bits in the range
     * described by start_bit and bit_length. The maximum for bit_length is 64, but for unaligned
     * values the range may exceed 8 bytes. In this case, the required ninth byte will be copied to
     * ninth_byte.
     *
     ...
     * @param [out] bytes_to_read Number of bytes that need to be copied to attain all requested
     * bits.
     *
     * @return Returns a standard result code.
     */
...
private:
     ...
     * The variable to read from or into might be too small and the accessed region of the memory
     * buffer might be out of range.
     *
     * @param [in]  start_bit     Bit position to start reading from. The least significant bit
     *                            has the index 0.
     * @param [in]  bit_length    Number of bits to read.
