...
a_util::result::Result MapAssignment::loadFromDOM(const a_util::xml::DOMElement& oAssignment,
                                                  MapErrorList& lstErrors)
{
    ...
    if (itAttrFrom == mapAttrs.end() && itAttrConst == mapAttrs.end() &&
        itAttrFct == mapAttrs.end()) {
        lstErrors.push_back(a_util::strings::format(
            "Missing <from>, <constant> or <function> attribute for <assignment> to '%s'",
            itAttrTo->second.c_str()));
        return ERR_INVALID_ARG;
    }
    if (itAttrFrom != mapAttrs.end() && itAttrConst != mapAttrs.end()) {
        lstErrors.push_back(a_util::strings::format(
            "<from> and <constant> attributes set at the same time for <assignment> to '%s'",
            itAttrTo->second.c_str()));
        return ERR_INVALID_ARG;
    }
    if (itAttrFrom != mapAttrs.end() && itAttrFct != mapAttrs.end()) {
        lstErrors.push_back(a_util::strings::format(
            "<from> and <function> attributes set at the same time for <assignment> to '%s'",
            itAttrTo->second.c_str()));
        return ERR_INVALID_ARG;
    }
    if (itAttrFct != mapAttrs.end() && itAttrConst != mapAttrs.end()) {
        lstErrors.push_back(a_util::strings::format(
            "<function> and <constant> attributes set at the same time for <assignment> to '%s'",
            itAttrTo->second.c_str()));
        return ERR_INVALID_ARG;
    }
    ...
    if (itAttrFrom != mapAttrs.end()) {
        if (itAttrFrom->second.empty()) {
            lstErrors.push_back(a_util::strings::format(
                "<from> attribute empty for <assignment> to '%s'", itAttrTo->second.c_str()));
            return ERR_INVALID_ARG;
        }
        ...
        if (itAttrFct->second.empty()) {
            lstErrors.push_back(
                a_util::strings::format("<function> attribute empty for <assignment> \
                                              to '%s'",
                                        itAttrTo->second.c_str()));
            return ERR_INVALID_ARG;
        }
        ...
        if (nIdx1 == std::string::npos || nIdx2 == std::string::npos || nIdx1 == 0 || nIdx2 == 0 ||
            nIdx1 > nIdx2 ||
            !(_function.find("simulation_time") == 0 || _function.find("trigger_counter") == 0 ||
              _function.find("received") == 0)) {
            lstErrors.push_back(
                a_util::strings::format("<function> should be of type 'simulation_time()', \
        ...
        if (_function.find("simulation_time") == 0) {
            if (nLength > 0) {
                lstErrors.push_back(a_util::strings::format(
                    "<function> of type 'simulation_time()' takes no argument for <assignment> \
                                                 to '%s'",
                    itAttrTo->second.c_str()));
                return ERR_INVALID_ARG;
            }
