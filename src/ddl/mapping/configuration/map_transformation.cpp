...
a_util::result::Result MapTransformationBase::createFromDOM(MapConfiguration* pConfig,
                                                            const a_util::xml::DOMElement& oTrans,
...
{
    MapEnumTableTransformation* pMapETransf = dynamic_cast<MapEnumTableTransformation*>(this);
    if (pMapETransf) {
...
MapPolynomTransformation::MapPolynomTransformation(MapConfiguration* pConfig,
                                                   const std::string& name)
...
bool MapPolynomTransformation::isEqual(const MapTransformationBase& oOther) const
{
    ...
    const MapPolynomTransformation* p = dynamic_cast<const MapPolynomTransformation*>(&oOther);
    if (p) {
        return getA() == p->getA() && getB() == p->getB() && getC() == p->getC() &&
               getD() == p->getD() && getE() == p->getE();
    }
    ...
    for (int i = 0; i < 5; ++i, c++) {
        if (!strCoefs[i].empty()) {
            if (!a_util::strings::isDouble(strCoefs[i])) {
                _config->appendError(
                    a_util::strings::format("Invalid coefficient attribute '%c' for polynomial "
                                            "transformation '%s' (expected number)",
                                            c,
                                            this->getName().c_str()));
                res = ERR_INVALID_ARG;
            }
...
a_util::result::Result MapPolynomTransformation::writeToDOM(
    a_util::xml::DOMElement& oDOMElement) const
{
...
bool MapEnumTableTransformation::isEqual(const MapTransformationBase& oOther) const
{
    ...
    const MapEnumTableTransformation* p = dynamic_cast<const MapEnumTableTransformation*>(&oOther);
    if (p) {
        return getDefaultStr() == p->getDefaultStr() &&
               getConversionsStr() == p->getConversionsStr();
    }

    return false;
}

a_util::result::Result MapEnumTableTransformation::loadFromDom(
    const a_util::xml::DOMElement& oTrans)
{
    const a_util::xml::DOMAttributes mapAttrs = oTrans.getAttributes();
    ...
    if (itAttrFrom == mapAttrs.end() || itAttrFrom->second.empty()) {
        _config->appendError(a_util::strings::format(
            "Missing <from> attribute for <transformation> '%s'", getName().c_str()));
    ...
    if (itAttrTo == mapAttrs.end() || itAttrTo->second.empty()) {
        _config->appendError(a_util::strings::format(
            "Missing <to> attribute for <transformation> '%s'", getName().c_str()));
        nRes = ERR_INVALID_ARG;
    }
    ...
    if (isOk(nRes)) {
        ...
        if (itAttrDefault == mapAttrs.end() || itAttrDefault->second.empty()) {
            _config->appendError(a_util::strings::format(
                "Missing <default> attribute for <transformation> '%s'", getName().c_str()));
            return ERR_INVALID_ARG;
        }
        ...
        for (a_util::xml::DOMElementList::const_iterator it = lstConv.begin(); it != lstConv.end();
             ++it) {
            a_util::result::Result res = a_util::result::SUCCESS;
            ...
            if (itConvAttrFrom == mapConvAttrs.end() || itConvAttrFrom->second.empty()) {
                _config->appendError(a_util::strings::format(
                    "Missing <from> attribute for <conversion> in <transformation> '%s'",
                    getName().c_str()));
                res = ERR_INVALID_ARG;
            }
            ...
            if (itConvAttrTo == mapConvAttrs.end() || itConvAttrTo->second.empty()) {
                _config->appendError(a_util::strings::format(
                    "Missing <to> attribute for <conversion> in <transformation> '%s'",
                    getName().c_str()));
                res = ERR_INVALID_ARG;
            }
...
a_util::result::Result MapEnumTableTransformation::writeToDOM(
    a_util::xml::DOMElement& oDOMElement) const
{
    ...
    for (MapStrConversionList::const_iterator itConv = _conversions.begin();
         itConv != _conversions.end();
         itConv++) {
...
a_util::result::Result ddl::mapping::MapEnumTableTransformation::setEnums(
    const std::string& strEnumFrom, const std::string& strEnumTo)
{
...
const MapEnumTableTransformation::ConversionMap& MapEnumTableTransformation::getConversions() const
{
...
{
    const ConversionMap::const_iterator it = _conversions_int.find((int64_t)f64Value);
...
a_util::result::Result MapEnumTableTransformation::setEnumsStr(const std::string& strEnumFrom,
                                                               const std::string& strEnumTo)
{
...
a_util::result::Result MapEnumTableTransformation::addConversionStr(const std::string& strFrom,
...
a_util::result::Result MapEnumTableTransformation::convertValuesWithDDL()
{
    ...
    // Search <from> in DataDefinition
    auto enum_type_from = _config->getDD()->getEnumTypes().get(_enum_from);
    if (!enum_type_from) {
        _config->appendError(a_util::strings::format(
            "<from> attribute for <transformation> '%s' should be an enumeration",
            getName().c_str()));
        _is_valid = false;
        return ERR_INVALID_TYPE;
    }
    // Search <to> in DataDefinition
    auto enum_type_to = _config->getDD()->getEnumTypes().get(_enum_to);
    if (!enum_type_to) {
        _config->appendError(a_util::strings::format(
            "<to> attribute for <transformation> '%s' should be an enumeration",
            getName().c_str()));
        _is_valid = false;
        return ERR_INVALID_TYPE;
    }

    // Search <default> in DataDefinition
    if (!_default_value.empty()) {
        auto value_found = enum_type_to->getElements().get(_default_value);
        if (!value_found || !a_util::strings::isInt64(value_found->getValue())) {
            _config->appendError(
                a_util::strings::format("<default> attribute for <transformation> '%s' should be "
                                        "an element of '%s' <enumeration>",
                                        getName().c_str(),
                                        _enum_to.c_str()));
            _is_valid = false;
            return ERR_INVALID_TYPE;
        ...
        _default_int = a_util::strings::toInt64(value_found->getValue());
    }

    a_util::result::Result res = a_util::result::SUCCESS;
    _conversions_int.clear();
    for (std::map<std::string, std::string>::iterator it = _conversions.begin();
         it != _conversions.end();
         ++it) {
        std::string strFromVal, strToVal;
        auto value_found = enum_type_from->getElements().get(it->first);
        if (!value_found || !a_util::strings::isInt64(value_found->getValue())) {
            _config->appendError(a_util::strings::format(
                "<from> attribute '%s' for <conversion> in <transformation> '%s' should be a "
                "element of '%s' <enumeration>",
                it->first.c_str(),
                getName().c_str(),
                enum_type_from->getName().c_str()));
            _is_valid = false;
            res = ERR_INVALID_TYPE;
        }
        else {
            strFromVal = value_found->getValue();
        }

        value_found = enum_type_to->getElements().get(it->second);
        if (!value_found || !a_util::strings::isInt64(value_found->getValue())) {
            _config->appendError(
                a_util::strings::format("<to> attribute '%s' for <conversion> in <transformation> "
                                        "'%s' should be a element of '%s' <enumeration>",
                                        it->second.c_str(),
                                        getName().c_str(),
                                        enum_type_to->getName().c_str()));
            _is_valid = false;
            res = ERR_INVALID_TYPE;
        }
        else {
