...
MapTarget::MapTarget(MapConfiguration* pConfig, std::string name, std::string type_name)
...
{
    _name = oOther._name;
    _type_name = oOther._type_name;
    _assignments = oOther._assignments;
    _sources = oOther._sources;
    _is_valid = oOther._is_valid;
    _config = oOther._config;
    for (MapTriggerList::const_iterator it = oOther._triggers.begin(); it != oOther._triggers.end();
         ++it) {
...
a_util::result::Result MapTarget::addAssignment(const MapAssignment& oAssignment)
{
    ...
    if (!_is_valid) {
        _config->appendError(a_util::strings::format(
            "Target Signal %s is not valid, cannot add any assignment", _name.c_str()));
        return ERR_INVALID_STATE;
    }
...
a_util::result::Result MapTarget::removeTrigger(MapTriggerBase* pTrigger)
{
    ...
    if (!strSource.empty()) {
        ...
        for (MapAssignmentList::const_iterator it = _assignments.begin(); it != _assignments.end();
             it++) {
...
a_util::result::Result MapTarget::addAssignmentNoTypeCheck(const MapAssignment& oAssignment)
{
    ...
    if (strTo.empty()) {
        _config->appendError(a_util::strings::format(
            "Target element name is empty in <target> '%s'", _name.c_str()));
        return ERR_INVALID_ARG;
    }

    for (MapAssignmentList::iterator itAssign = _assignments.begin();
         itAssign != _assignments.end();
         itAssign++) {
    ...
    if (oAssignment.getSource().empty() && oAssignment.getConstant().empty() &&
        oAssignment.getFunction().empty()) {
        _config->appendError(
            a_util::strings::format("Missing <from>, <constant> or <function> attribute for "
    ...
    if (!oAssignment.getConstant().empty() &&
        ...
        _config->appendError(a_util::strings::format(
            "Empty or non-numeric constant for <assignment> to '%s' in <target> '%s'",
            strTo.c_str(),
            _name.c_str()));
        return ERR_INVALID_ARG;
    }
    ...
    if (!oAssignment.getModulo().empty()) {
        if (!a_util::strings::isInt64(oAssignment.getModulo()) ||
            ...
            _config->appendError(
                a_util::strings::format("Non-integer or zero modulo in transmission_counter for "
                                        "<assignment> to '%s' in <target> '%s'",
                                        strTo.c_str(),
                                        _name.c_str()));
            return ERR_INVALID_ARG;
    ...
    if (!oAssignment.getSource().empty()) {
        for (MapSourceList::const_iterator itSource = _config->getSourceList().begin();
             itSource != _config->getSourceList().end();
             ++itSource) {
...
a_util::result::Result MapTarget::modifySourceName(const std::string& name,
                                                   const std::string& strNewName)
{
    ...
    for (MapTriggerList::iterator itTrigger = _triggers.begin(); itTrigger != _triggers.end();
         itTrigger++) {
...
a_util::result::Result MapTarget::loadFromDOM(const a_util::xml::DOMElement& oTarget)
{
    ...
    for (a_util::xml::DOMElementList::const_iterator it = lstElements.begin();
        ...
        if (name == "assignment") {
            ...
            _config->appendError(
                a_util::strings::format("Invalid element type in <target> '%s': '%s'",
                                        itName->second.c_str(),
                                        name.c_str()));
...
a_util::result::Result MapTarget::writeToDOM(a_util::xml::DOMElement& oDOMElement) const
{
    ...
    for (MapTriggerList::const_iterator itTrigger = _triggers.begin(); itTrigger != _triggers.end();
         itTrigger++) {
...
a_util::result::Result MapTarget::checkDoubleAssignments()
{
    ...
    for (MapAssignmentList::iterator itAssign = _assignments.begin();
         itAssign != _assignments.end();
         itAssign++) {
        for (std::set<std::string>::iterator itParent = oAssignedElList.begin();
             itParent != oAssignedElList.end();
             itParent++) {
            ...
            if (strEl.find(*itParent) == 0) {
                ...
                if (strEl.find('.') == 0 || strEl.find('[') == 0) {
                    _config->appendError(
                        a_util::strings::format("<assignment> to '%s' not possible because of "
                                                "<assignment> to %s in <target> '%s'",
                                                strEl.c_str(),
                                                itParent->c_str(),
                                                getName().c_str()));
...
void MapTarget::swap(MapTarget& oOther)
{
    ...
    swap(_type_name, oOther._type_name);
...
bool ddl::mapping::operator==(const MapTarget& a, const MapTarget& b)
{
    ...
    for (; itA != lstTriggerA.end() && itB != lstTriggerB.end(); ++itA, ++itB) {
        if (!(*itA)->isEqual(*(*itB))) {
            return false;
        }
    }
...
a_util::result::Result MapTarget::setType(const std::string& type_name)
{
    if (!_config) {
        return ERR_INVALID_STATE;
    }
